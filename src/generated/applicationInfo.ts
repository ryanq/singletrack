...
export const ApplicationInfo: MessageFns<ApplicationInfo> = {
  encode(message: ApplicationInfo, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.platform !== 0) {
      writer.uint32(8).int32(message.platform);
    }
    if (message.platformVersion !== undefined) {
      Version.encode(message.platformVersion, writer.uint32(18).fork()).join();
    }
    if (message.application !== 0) {
      writer.uint32(24).int32(message.application);
    }
    if (message.applicationVersion !== undefined) {
      Version.encode(message.applicationVersion, writer.uint32(34).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): ApplicationInfo {
