...
export const PresentationSlide: MessageFns<PresentationSlide> = {
  encode(message: PresentationSlide, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.baseSlide !== undefined) {
      Slide.encode(message.baseSlide, writer.uint32(10).fork()).join();
    }
    if (message.notes !== undefined) {
      PresentationSlide_Notes.encode(message.notes, writer.uint32(18).fork()).join();
    }
    ...
    if (message.chordChart !== undefined) {
      URL.encode(message.chordChart, writer.uint32(34).fork()).join();
    }
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): PresentationSlide {
  ...
  toJSON(message: PresentationSlide): unknown {
    ...
    if (message.templateGuidelines?.length) {
      obj.templateGuidelines = message.templateGuidelines.map((e) => AlignmentGuide.toJSON(e));
    }
    if (message.chordChart !== undefined) {
...
export const PresentationSlide_Notes: MessageFns<PresentationSlide_Notes> = {
  encode(message: PresentationSlide_Notes, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.rtfData.length !== 0) {
      writer.uint32(10).bytes(message.rtfData);
    }
    if (message.attributes !== undefined) {
      Graphics_Text_Attributes.encode(message.attributes, writer.uint32(18).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): PresentationSlide_Notes {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
